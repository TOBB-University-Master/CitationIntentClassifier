...
def find_best_trial_by_rerunning_validation(model_name, model_checkpoint_dir, device):
    ...
    # train_v3'ün klasör isimlendirmesi: trial_1_bert-base-turkish-cased
    ...
    for trial_dir_name in tqdm(trial_dirs, desc=f"Finding best trial for {model_name.split('/')[-1]}"):
        ...
        try:
            ...
            train_val_dataset, _ = random_split(full_dataset, [train_val_size, len(full_dataset) - train_val_size],
                                                generator=generator)
            val_dataset, _ = random_split(train_val_dataset, [int(0.85 * len(train_val_dataset)),
                                                              len(train_val_dataset) - int(
                                                                  0.85 * len(train_val_dataset))], generator=generator)
...
def main():
    ...
    with open(log_file_path, 'w', encoding='utf-8') as log_file:
        ...
        for model_name in MODELS:
            ...
            log_and_print("\n--- TEST SONUÇLARI (CONTEXT-AWARE HİYERARŞİK) ---", log_file)
            ...
            plt.title(f'Confusion Matrix (Context-Aware Hierarchical) - {model_short_name}', fontsize=16)
        ...
        log_and_print("\n\n" + "=" * 60, log_file)
        log_and_print("     TÜM MODELLERİN ÖZET SONUÇLARI (Experiment 1.3 - Context-Aware Hierarchical)", log_file)
        ...
        ax.set_title('Experiment 1.3: Context-Aware Hierarchical Model Karşılaştırması', fontsize=16, fontweight='bold')
